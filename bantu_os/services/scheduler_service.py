...
class SchedulerService:
        ...
        *,
        ...
        with self._lock:
            ...
            conn.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (
                task.id, task.name, task.schedule_type, task.schedule_value,
                task.action, json.dumps(task.params), task.status,
                task.created_at, task.next_run
            ))
            conn.commit()
            conn.close()
    ...
    def cancel(self, task_id: str) -> Dict[str, Any]:
        ...
        with self._lock:
            conn = sqlite3.connect(self.db_path)
            cur = conn.execute('SELECT id, name, status FROM tasks WHERE id = ?', (task_id,))
            row = cur.fetchone()
            
            if not row:
                raise ValueError(f'Task {task_id} not found')
            
            conn.execute('UPDATE tasks SET status = ? WHERE id = ?', ('cancelled', task_id))
            conn.commit()
            conn.close()
        
        self._log_operation('cancel', task_id, row[1])
        
        return {'task_id': task_id, 'status': 'cancelled', 'timestamp': datetime.now().isoformat()}
    ...
    def get_task(self, task_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            'id': row[0], 'name': row[1], 'schedule_type': row[2],
            'schedule_value': row[3], 'action': row[4],
            'params': json.loads(row[5]), 'status': row[6],
            'created_at': row[7], 'next_run': row[8],
            'last_run': row[9], 'run_count': row[10], 'error': row[11],
        }
    ...
    def list_tasks(self, status: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        return [
            {
                'id': r[0], 'name': r[1], 'schedule_type': r[2],
                'schedule_value': r[3], 'action': r[4],
                'params': json.loads(r[5]), 'status': r[6],
                'created_at': r[7], 'next_run': r[8],
                'last_run': r[9], 'run_count': r[10], 'error': r[11],
            }
            for r in rows
        ]
    ...
    def record_run(
        ...
        with self._lock:
            ...
            cur = conn.execute('SELECT schedule_type, schedule_value FROM tasks WHERE id = ?', (task_id,))
            row = cur.fetchone()
    ...
    def get_task_history(self, task_id: str, limit: int = 20) -> List[Dict[str, Any]]:
        ...
        return [
            {
                'id': r[0], 'task_id': r[1], 'started_at': r[2],
                'completed_at': r[3], 'status': r[4],
                'result': r[5], 'error': r[6],
            }
            for r in rows
        ]
