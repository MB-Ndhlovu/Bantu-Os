...
class SchedulingAgent:
    ...
    # -------------------- Public API --------------------
    def add_event(
        self, title: str, when_str: str, now: Optional[datetime] = None
    ) -> int:
    ...
    def list_events(self) -> List[Event]:
        with sqlite3.connect(self.db_path) as conn:
            cur = conn.execute(
                "SELECT id, title, when_ts FROM events ORDER BY when_ts ASC"
            )
            out: List[Event] = []
...
IN_X = re.compile(
    r"\bin\s+(?P<num>\d+)\s+(?P<unit>minute|minutes|hour|hours)\b", re.IGNORECASE
)
