...
class TestRustShell:
    ...
    def shell_binary(self) -> Path:
        ...
        build_result = subprocess.run(
            ["cargo", "build", "--release", "--manifest-path", str(SHELL_DIR / "Cargo.toml")],
            capture_output=True,
            text=True,
            timeout=120,
        )
        ...
        if not binary.exists():
            pytest.fail(f"Expected binary not found after build in {SHELL_DIR / 'target'}")
    ...
    def test_shell_runs_and_shows_banner(self, shell_binary: Path):
        ...
        assert "Bantu-OS Shell" in stdout, f"Expected banner not found in output:\n{stdout}"
    ...
    def test_exit_command_terminates_cleanly(self, shell_binary: Path):
        ...
        assert proc.returncode == 0, f"Shell should exit cleanly after 'exit' command: {stderr}"
    ...
    def test_unknown_command_returns_error_line(self, shell_binary: Path):
        ...
        assert "You entered: badcommand" in stdout, (
